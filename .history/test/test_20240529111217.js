...
describe("deploying and testing smart contract....'",  () => {
  ...
  it('Consultant sends points to client 1 account', async () => {
    await ar.connect(deployer).sendPoints('1000',client1)
  })
  it('Stops clients from add more points to their account', async () => {
    await expect(ar.connect(client1).sendPoints(100, client1)).to.be.revertedWith('you are not a consultant.')
  })
  it('updates a crypto address to a string(name)', async () => {
    await ar.connect(deployer).addClientName(client1,'eyan')
    console.log(await ar.mappingPoints[0])
  })
